	...
	$.Redactor.opts = {
		...
		langs: {
			en: {
				...
				class_left_value: 'img-left',
				class_right_value: 'img-right',
				class_full_value: 'img-full-width',
		...
		block: function()
		{
			return {
				...
				format: function(tag, type, value)
				{
					...
					var formatTags = ['p', 'div', 'mark', 'img', 'pre', 'blockquote', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'];
				...
				build: function(name, $dropdown, dropdownObject)
				{
					if (name == 'formatting' && this.opts.formattingAdd)
					{
						$.each(this.opts.formattingAdd, $.proxy(function(i,s)
						{
							...
							if (this.opts.linebreaks && s.type == 'block' && (s.tag == 'p' || s.tag == 'div')) return;
		...
		image: function()
		{
			return {
				...
				showEdit: function($image)
				{
					...
					var classValue = ($image.attr('class')) ? $image.attr('class') : '';
					$('#redactor-image-class').val(classValue);
					this.modal.show();
				...
				/********************************* Добавляем класс к картинке *********************************/
				setClassing: function($image)
				{
					var classing = $('#redactor-image-class').val();
					$image.attr('class', classing);
				},
				/********************************* /Добавляем класс к картинке *********************************/
				...
				loadTemplates: function()
				{
					this.opts.modal = {
						...
						+ '<section id="redactor-modal-image-edit">'
							...
							/********************************* Добавляем класс к картинке *********************************/
							+ '<label class="redactor-image-class-option">' + this.lang.get('image_class') + '</label>'
							+ '<select class="redactor-image-class-option" id="redactor-image-class" aria-label="' + this.lang.get('image_class') + '">'
								+ '<option value="">' + this.lang.get('none') + '</option>'
								+ '<option value="' + this.lang.get('class_left_value') + '">' + this.lang.get('class_left') + '</option>'
								+ '<option value="' + this.lang.get('class_right_value') + '">' + this.lang.get('class_right') + '</option>'
								+ '<option value="' + this.lang.get('class_full_value') + '">' + this.lang.get('class_full') + '</option>'
							+ '</select>'
							/********************************* /Добавляем класс к картинке *********************************/
